...
async function generateWithGoogleImagen3(prompt, size, quality) {
  ...
  // --- START OF THE OFFICIAL FIX from Google's Documentation ---
  // The endpoint uses :predict and the payload structure is different.
  ...
  const response = await fetch(correctUrl, {
    // --- END OF THE OFFICIAL FIX ---
    method: 'POST',
    headers: {
      'Content-Type': 'application/json'
    },
    body: JSON.stringify(requestBody) // Use the new request body structure
  });
  if (!response.ok) {
  ...
  // The response structure is also different. We need to get the base64 string from predictions.
